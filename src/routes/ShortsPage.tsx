...
export function ShortsPage() {
  ...
  const premiumOnly = search.get('premium') === '1';
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setError(null);
      try {
        const queue = await loadShortsRuntimeQueue(topic, contentId || null, originHint, stateItem, { freeOnly, premiumOnly });
        if (!active) return;
  ...
  }, [contentId, freeOnly, originHint, premiumOnly, stateItem, topic]);
  ...
  }, [activeItem, freeOnly, premiumOnly, topic]);
